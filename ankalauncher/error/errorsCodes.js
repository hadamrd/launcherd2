...
const MAP_CODE_TO_ERR = (() => {
    const result = {};
    return function fillRec(dict) {
        for (const key of Object.keys(dict)) dict[key].code ? result[dict[key].code] = {
            message: dict[key].message
        } : fillRec(dict[key])
    }(codeErrors), result
})();
export function getByCodeError(errCode) { return MAP_CODE_TO_ERR[errCode]; }
